...
- **`src/services/api`** — a named-export `fetch` wrapper (`login`, `getPlayer`, `getTerritory`, `submitAnswer`, `getTradeOffers`, `acceptTradeOffer`, ...), not an axios-style `api.get`/`api.post` client. Base URLs are **hardcoded** in `src/services/api/base_url.js` (no `.env`/`import.meta.env` usage anywhere in the frontend) — changing the backend target requires editing that file and rebuilding.
